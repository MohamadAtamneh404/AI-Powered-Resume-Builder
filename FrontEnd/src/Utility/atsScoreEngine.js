 ...
 * Synchronously analyzes resume data and gives live ATS score and telemetry.
 */
...
const METRIC_PATTERN =
  /(?:\$|€|£)?\b\d+(?:[.,]\d+)?\s*(?:%|k|m|b|x|\+|users|clients|customers|requests|transactions|ms|seconds|minutes|hours|days|weeks|months|years|members|teams|projects|dollars)?\b/i;
...
export function calculateAtsScore(resume) {
  ...
  const summary =
    basics.summary ||
    (resume.blocks &&
      resume.blocks.find((b) => b.type === "summary")?.content) ||
    "";
  ...
  if (Array.isArray(education) && education.length > 0) {
    const validEdu = education.filter(
      (e) => e.institution || e.degree || e.studyType,
    );
  ...
  let totalScore =
    contactScore + sectionScore + impactScore + verbScore + skillScore;

  // Penalty for placeholder text
  const resumeJsonString = JSON.stringify(data);
  const hasPlaceholders =
    /jane\.doe|john\.doe|email@example\.com|\+1 234 567|your name/i.test(
      resumeJsonString,
    );
  if (hasPlaceholders) {
  ...
  if (contactScore >= 18) {
    goodParts.push({
      ...
      description:
        "Name, email, phone, location, and professional links are fully populated and parse-safe.",
      points: contactScore,
      maxPoints: 20,
    });
  } else {
    if (!basics.email || !basics.email.includes("@")) {
      badParts.push({
        ...
        suggestion:
          "Add your direct personal or professional email for recruiter outreach.",
        fixScope: "header",
    ...
    if (!basics.location) {
      badParts.push({
        ...
        suggestion:
          "Add your current city and country to pass geographic ATS search filters.",
        fixScope: "header",
        actionLabel: "Fix Contact Info",
        pointsLost: 3,
      });
    ...
    if (
      ...
      badParts.push({
        ...
        suggestion:
          "Add your LinkedIn or GitHub profile to elevate candidate credibility.",
        fixScope: "header",
  ...
  if (wordCount >= 15) {
    ...
    badParts.push({
      id: "summary",
      title:
        wordCount === 0 ? "Missing Professional Summary" : "Summary Too Brief",
      issue:
        wordCount === 0
          ? "No summary section found."
          : `Summary is only ${wordCount} words (minimum 15 recommended).`,
      suggestion:
        "Add a 2–3 sentence executive summary highlighting your core skills, years of experience, and primary domain expertise.",
      fixScope: "summary",
  ...
  const validRoles = Array.isArray(work)
    ? work.filter((w) => w.title || w.position || w.company)
    : [];
    ...
    badParts.push({
      ...
      issue:
        validRoles.length === 0
          ...
          : "Only 1 work experience entry found.",
      suggestion:
        "Include at least 2 relevant roles, internships, or freelance projects.",
      fixScope: "work",
  ...
  // 4. Education Audit
  const validEdu = Array.isArray(education)
    ? education.filter((e) => e.institution || e.degree || e.studyType)
    : [];
  if (validEdu.length >= 1) {
    ...
    badParts.push({
      ...
      suggestion:
        "Add your college degree, high school, or certification program.",
      fixScope: "education",
  ...
  if (metricBulletsCount >= 3) {
    ...
    badParts.push({
      ...
      suggestion:
        "Enrich your work bullets with quantifiable outcomes: % growth, revenue figures, time saved, or team sizes.",
      fixScope: "work",
  ...
  if (actionVerbBulletsCount >= 2 && verbRatio >= 0.4) {
    ...
    badParts.push({
      ...
      issue:
        "Few bullets begin with strong action verbs (e.g., spearheaded, architected, engineered, increased).",
      suggestion:
        "Start each accomplishment bullet with a strong past-tense action verb.",
      fixScope: "work",
  ...
  if (skillCount >= 8) {
    ...
    badParts.push({
      ...
      suggestion:
        "Add technical competencies, programming tools, methodologies, and platforms.",
      fixScope: "skills",
  ...
  if (hasPlaceholders) {
    badParts.push({
      ...
      issue:
        "Found dummy placeholder data like 'jane.doe@example.com' or 'Your Name'.",
      suggestion:
        "Replace all sample template placeholder values with your actual candidate credentials.",
      fixScope: "header",
